...
public class herramientas_2{

    // Se definen los atributos

    // Estos son los operandos con los que se trabaja en decimal 
    Double op1; 
    Double op2; 
    // Estos son los operandos con los que se trabaja en binario 
    int op1_b;
    int op2_b;
    // La memoria que tiene la calculadora en modo decimal
    Double MEM;
    // La memoria que tiene la calculadora en modo binario
    int MEM_b;
    // Es para prender o apagar la calculadora
    boolean prendida;

    // Se hace un constructor vacío
    public herramientas_2(){}
    
    // Se definen los gets 
    public Double get_op1(){return op1; }
    public Double get_op2(){return op2; }
    public int get_op1_b(){return op1_b; }
    public int get_op2_b(){return op2_b; }
    public Double get_MEM(){return MEM; }
    public int get_MEM_b(){return MEM_b; } 
    public boolean get_prendida(){return prendida;}

    // Se definen los sets 
    public void set_op1(Double nuevo_op1){ op1 = nuevo_op1; }
    public void set_op2(Double nuevo_op2){ op2 = nuevo_op2; }
    public void set_op1_b(int nuevo_op1_b){ op1_b = nuevo_op1_b; }
    public void set_op2_b(int nuevo_op2_b){ op2_b = nuevo_op2_b; }
    public void set_MEM(Double nuevo_MEM){ MEM = nuevo_MEM; }
    public void set_MEM_b(int nuevo_MEM_b){ MEM_b = nuevo_MEM_b; }
...
// ############################################# MÉTODOS GENERALES (QUE SE USAN EN DECIMAL Y EN BINARIO) #############################################
    ...
    // Este método se usa para guardar el valor de una operación en memoria si así lo quiere el usuario en el sistema decimal 
    public void guardar_memoria_decimal(Scanner memoria, Double resultado){
    ...
    // Este método se usa para guardar el valor de una operación en memoria si así lo quiere el usuario en el sistema binario 
    public void guardar_memoria_binario(int resultado){
        ...
        // Mensaje donde se muestra el valor actual que tiene la memoria (aributo MEM)
        System.out.println("\nEl valor que se tiene almacenado en la memoria es: " + this.get_MEM_b() + "\n");
    }
    ...
    public void esquema(){
        ...
        while(this.get_prendida()==true){
            ...
            if(modo==1){
                ...
                this.procedimiento_decimal(operando1, operando2);
            }
 ...
 // ############################################# MÉTODOS QUE SE USAN PARA EL SISTEMA DECIMAL #############################################
    ...
    // Éstos se ingresan como String pero después se convierten en Double, bajo ciertas condiciones 
    ...
    public void operandos_decimal(Scanner operando1, Scanner operando2){
        ...
        // Si el usuario NO escribe la palabra MEM entonces se convierte en un valor de tipo Double el número que escribe (recordemos que es un String de principio)
            Double op1_int = Double.parseDouble(op_1); 
        ...
        if(op_2.equals(mem) || op_2.equals(MEM)){
            ...
            Double op2_int = Double.parseDouble(op_2);
    ...
    public void procedimiento_decimal(Scanner operando1, Scanner operando2){    
        ...
        switch(operacion){
            ...
            case 1:        
                ...
                Double resultado_suma = this.get_op1() + this.get_op2();
                System.out.println("\nEl resultado es: " + resultado_suma);
                // Se pregunta al usuario si quiere guardar el resultado en memoria        
                this.guardar_memoria_decimal(entrada_usuario, resultado_suma);
                break;            
            ...
            case 2: 
                ...
                Double resultado_resta = this.get_op1() - this.get_op2();
                System.out.println("\nEl resultado es: " + resultado_resta);
                // Se pregunta al usuario si quiere guardar el resultado en memoria        
                this.guardar_memoria_decimal(entrada_usuario, resultado_resta);
                break;
            ...
            case 3: 
                ...
                Double resultado_multiplicacion = this.get_op1() * this.get_op2();
                System.out.println("\nEl resultado es: " + resultado_multiplicacion);
                // Se pregunta al usuario si quiere guardar el resultado en memoria        
                this.guardar_memoria_decimal(entrada_usuario, resultado_multiplicacion);
                break;
...
// #####################################################################################################################################
 ...
 // ############################################# MÉTODOS QUE SE USAN PARA EL SISTEMA BINARIO #############################################
    ...
    // Los operandos se ingresan como String pero después se convierten en int, bajo ciertas condiciones 
    // Los objetos que ingresa el usuario son de tipo Scanner
    public void operandos_binario(){
        // Se convierte el string "MEM" en minúsculas
        String MEM = "MEM";
        String mem = MEM.toLowerCase();
        ...
        if(op_1.equals(mem) || op_1.equals(MEM)){
        // Si el usuario escribe MEM se hace lo siguiente: 
        // El atributo op1_b recibe el valor que está almacenado en la memoria y por defecto, MEM tiene el valor 0 
            this.set_op1_b(this.get_MEM_b());
            }

        // Esto es por si el usuario ingresa un número que no está compuesto por 0's y 1's
        // A grandes rasgos, si el usuario ingresa un número que no está compuesto únicamente por 0's y/o 1's se reinicia el programa
        else{
            ...
            || op_1.contains(String.valueOf("6")) || op_1.contains(String.valueOf("7")) || op_1.contains(String.valueOf("8")) || op_1.contains(String.valueOf("9"))) {
                ...
                System.out.println("\n¡¡¡ E R R O R !!! \nEscribiste un número inválido.");
                System.out.println("Recuerda que en el sistema binario solo existen 0's y 1's. Escribe un número que contenga solo 0's y/o 1's. ");
                System.out.println("Empieza de nuevo todo, por favor.\nLamentamos las molestias :(\n");
                // Se reinicia todo el programa
                this.esquema();
                }
            ...
            // Si el usuario no escribe la palabra MEM entonces se convierte en un valor entero el número que escribe (recordemos que es un String de principio)
                ...
                // El atributo op1_b recibe el valor que ingreso el usuario
                this.set_op1_b(op1_int);
            }
        }
    ...
    public void procedimiento_binario(){    
        // Se imprimen las operaciones con las cuales se puede trabajar
        this.print_operaciones();
        // Entrada del usuario
        Scanner entrada_usuario = new Scanner(System.in);
        ...
        switch(operacion){
            ...
            case 1:        
                ...
                this.operandos_binario();
                // Se transforman los valores ingresados a decimal usando el ḿétodo 'binario_a_decimal'
                int op1_decimal = this.binario_a_decimal(this.get_op1_b());
                int op2_decimal = this.binario_a_decimal(this.get_op2_b());
                ...
                this.guardar_memoria_binario(resultado_suma_binario);
                break;
            ...
            case 2: 
                ...
                int op3_decimal = this.binario_a_decimal(this.get_op1_b());
                int op4_decimal = this.binario_a_decimal(this.get_op2_b());
                // Como condición, el primer operando debe de ser un valor mayor al segundo operando 
                if(op3_decimal-op4_decimal<0){
                    System.out.println("\nU P S! Intenta que el primer valor sea mayor que el segundo.");
                    System.out.println("Se está trabajando para mejorar esto. \nEmpieza de nuevo.");
                    this.procedimiento_binario();
                    break;
                }
                else{
                    ...
                    // Se pregunta al usuario si quiere guardar el resultado en memoria        
                    this.guardar_memoria_binario(resultado_resta_binario);
                    break;
                }
            ...
            case 3:
                ...
                this.operandos_binario();
                // Se transforman los valores ingresados a decimal usando el método 'binario_a_decimal'
                int op5_decimal = this.binario_a_decimal(this.get_op1_b());
                int op6_decimal = this.binario_a_decimal(this.get_op2_b());
                ...
                // Se pregunta al usuario si quiere guardar el resultado en memoria 
                this.guardar_memoria_binario(resultado_producto_binario);
                break;
